...
**答辩参考**：技术栈/数据链路/算法原理/答辩Q&A 见《技术答辩参考文档.md》。
**三入口**：
...
| 项 | 内容 | 优先级 |
|---|---|---|
| 数字滚动动画 | ✅ 指标卡数字滚动（750ms 缓动，更新即滚动）| 中 ✅ |
...
| **布局重排（三栏）** | ✅ 左实时栏（告警/事件/健康度/充电效率）+ 中主图（趋势/地图/预测/走势/热力）+ 右指标栏（状态/TOP5/用户/峰谷/碳排）+ 底部可折叠 ML 区 | ✅ |
| **充电站地图升级** | ✅ 面板加高至 480；新增离线伪城区地图（Canvas 底图：道路/河流/网格，滚轮缩放、拖拽平移、双击复位、悬停提示、状态着色），与点阵模式可切换（面板右上角 地图/点阵 按钮）| ✅ |
| **左栏窄列修复** | ✅ 告警/事件行改两列紧凑布局防遮挡；健康度"健康X"span 类名 bug 修复（误套面板 height）；充电效率图窄列适配 | ✅ |
...
- [ ] 站点下钻 / 地图浮窗
- [x] 指标数字滚动动画（750ms 缓动）
- [x] 布局重排：三栏大屏（左实时栏/中主图/右指标栏）+ 底部可折叠 ML 区（折叠状态 localStorage 记忆）