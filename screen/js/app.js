(function () {
  ...
  let socket=null;
  const dashboard=new DashboardComponents.MainDashboard(filters => { DataStore.setFilters(filters); if (ScreenConfig.mode === "live") { /* HTTP 模式由 loadHiveData 重拉 */ } });
  ...
  if (ScreenConfig.mode === "live") {
    // 从网关 /api/snapshot 拉 Hive 聚合数据（WebSocket 服务端未运行，改用 HTTP）
    const loadHiveData = () => fetch("/api/snapshot").then(r => r.json()).then(payload => {
